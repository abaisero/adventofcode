#!/usr/bin/env ruby
# frozen_string_literal: true

require 'stringio'
require 'test/unit/assertions'
include Test::Unit::Assertions

def read_data(io)
  stacks = {}
  io.each do |line|
    next if /(\s+\d+)+/.match? line
    break if line.strip.empty?

    crate_indices = (1...line.length).step(4)
    crates = crate_indices.map { |i| line[i] }
    crates_with_indices = crates.each.with_index(1).reject { |crate, _index| crate == ' ' }

    crates_with_indices.each do |crate, index|
      stacks[index] = [] unless stacks.has_key? index
      stacks[index].unshift crate
    end
  end

  movements = io.map do |line|
    match = /^move (?<num>\d+) from (?<from>\d+) to (?<to>\d+)$/.match line
    [match['num'].to_i, match['from'].to_i, match['to'].to_i]
  end

  [stacks, movements]
end

def move_crates_9000(stacks, movements)
  movements.each do |num, from, to|
    num.times do
      stacks[to] << stacks[from].pop
    end
  end
end

def get_top_crates(stacks)
  stacks.keys.sort.map { |key| stacks[key].last }.join
end

def part1(io)
  stacks, movements = read_data io
  move_crates_9000 stacks, movements
  get_top_crates stacks
end

def move_crates_9001(stacks, movements)
  movements.each do |num, from, to|
    stacks[to].concat stacks[from].pop(num)
  end
end

def part2(io)
  stacks, movements = read_data io
  move_crates_9001 stacks, movements
  get_top_crates stacks
end

example = StringIO.open \
  "    [D]
[N] [C]
[Z] [M] [P]
 1   2   3

move 1 from 2 to 1
move 3 from 1 to 3
move 2 from 2 to 1
move 1 from 1 to 2"
assert_equal part1(example), 'CMZ'
example.rewind
assert_equal part2(example), 'MCD'

p part1 File.open('05.txt')
p part2 File.open('05.txt')
